...
const isLoginOrRegister = (apiUrl) => API_URL.findIndex((url) => apiUrl.includes(url)) >= 0;

const checkToken = (response) => {
    const apiUrl = get(response, "config.url");
    const token = isLoginOrRegister(apiUrl) && get(response, "data.tokens.access.token", false);
    return token;
};
